...
import com.example.mycast.recorder.AudioEncodeConfig;
import com.example.mycast.recorder.Notifications;
import com.example.mycast.recorder.ScreenRecorder;
import com.example.mycast.recorder.Utils;
import com.example.mycast.recorder.VideoEncodeConfig;

import com.example.mycast.recorder.view.NamedSpinner;
...
import static com.example.mycast.recorder.ScreenRecorder.AUDIO_AAC;
import static com.example.mycast.recorder.ScreenRecorder.VIDEO_AVC;
...
public class RecorderActivity extends Activity {
    ...
    private ScreenRecorder newRecorder(MediaProjection mediaProjection, VideoEncodeConfig video,
        ...
        ScreenRecorder r = new ScreenRecorder(video, audio, mediaProjection, output.getAbsolutePath());
    ...
    private void changeView(){
        mButton.setText("OK");
        mButton.setOnClickListener(this::onButtonClickConfirm);

        mAudioToggle.setVisibility(View.GONE);
    ...
    public static int __width = 1080;
    public static int __height = 1920;
    public static int __bitrate = 5000000;
    public static int __framerate = 30;
    public static int __iframe = 1;
    public static String __codec = "OMX.IMG.TOPAZ.VIDEO.Encoder";
    public static MediaCodecInfo.CodecProfileLevel __profileLevel = null;

    public static String __audioCodec = "OMX.google.aac.encoder";
    public static int __audioBitrate = 80000;
    public static int __samplerate = 44100;
    public static int __channelCount = 1;
    public static int __profile = 1;

    public static ScreenRecorder __recorder = null;
//    public static VirtualDisplay __virtualDisplay = null;

    private void onButtonClickConfirm(View v) {
        int[] selectedWithHeight = getSelectedWithHeight();
        boolean isLandscape = isLandscape();
        __width = selectedWithHeight[isLandscape ? 0 : 1];
        __height = selectedWithHeight[isLandscape ? 1 : 0];
        __bitrate = getSelectedVideoBitrate();
        __framerate = getSelectedFramerate();
        __iframe = getSelectedIFrameInterval();
//        __codec = getSelectedVideoCodec();
//        __profileLevel = getSelectedProfileLevel();

//        __audioCodec = getSelectedAudioCodec();
        __audioBitrate = getSelectedAudioBitrate();
//        __samplerate = getSelectedAudioSampleRate();
//        __channelCount = getSelectedAudioChannelCount();
//        __profile = getSelectedAudioProfile();

        finish();
    }

    public static void startCapturingStatic(MediaProjection mediaProjection) {
        ...
        __recorder = new ScreenRecorder(video, audio, mediaProjection, file.getAbsolutePath());
        __recorder.OUTER = true;
        __recorder.start();
    }

    public static void stopRecordingAndOpenFileStatic(Context context) {
        File file = new File(__recorder.getSavedPath());
        __recorder.quit();
        __recorder = null;
    ...
    private void onButtonClick(View v) {
        if (mRecorder != null) {
            ...
            toast(getString(R.string.no_permission_to_write_sd_card));
        }
    }
