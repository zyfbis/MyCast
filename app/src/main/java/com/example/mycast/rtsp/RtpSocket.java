...
import com.example.mycast.Tunnel.MySocket;
...
public class RtpSocket implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            while (mBufferCommitted.tryAcquire(4, TimeUnit.SECONDS)) {
                ...
                if (mCount++ > 30) sendPacket(mPackets[mBufferOut]);
    ...
    private void sendPacket(DatagramPacket packet) throws IOException {
        if (MainActivity.TheTunnelServer != null) {
            MySocket socket = MainActivity.TheTunnelServer.getByAddress(packet.getAddress());
            if (socket != null) {
                try {
                    if (socket.send(packet)) {
//                        Log.i(TAG, "sendPacket: send by Tunnel, sized: " + packet.getLength());
                        return;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    MainActivity.TheTunnelServer.removeSocket(socket);
                }
            }
        }

        mSocket.send(packet);
//        Log.i(TAG, "sendPacket: send by UDP, sized: " + packet.getLength());
    }
