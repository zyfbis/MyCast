...
public class ScreenRecorder {
    ...
    public ScreenRecorder(VideoEncodeConfig video,
                          ...
                          String dstPath) {
        mDstPath = dstPath;
        mVideoEncoder = new VideoEncoder(video, mediaProjection);
    ...
    private void writeSampleData(int track, MediaCodec.BufferInfo buffer, ByteBuffer encodedData) {
        if (OUTER) {
            if (track == mVideoTrackIndex) {
                writeVideoData(buffer, encodedData);
            } else if (track == mAudioTrackIndex) {
                writeAudioData(buffer, encodedData);
            }
        }
    ...
    public byte[] videoConfigByte = null;
    public byte[] audioConfigByte = null;
    public static long getTimeStamp(){
        return System.nanoTime();
    }

    public void writeVideoData(MediaCodec.BufferInfo buffer, ByteBuffer encodedData) {
        byte[] outData = new byte[buffer.size];
        encodedData.get(outData);

        if (buffer.flags == 2) {
            videoConfigByte = outData;
        } else if (buffer.flags == 1) {
            byte[] keyframe = new byte[buffer.size + videoConfigByte.length];
            ...
            MainActivity.putVideoData(keyframe, getTimeStamp());
        } else {
            MainActivity.putVideoData(outData, getTimeStamp());
        }
    }

    public void writeAudioData(MediaCodec.BufferInfo buffer, ByteBuffer encodedData) {
        byte[] outData = new byte[buffer.size];
        encodedData.get(outData);

        if (buffer.flags == 2) {
            audioConfigByte = outData;
        } else if (buffer.flags == 1) {
            byte[] keyframe = new byte[buffer.size + audioConfigByte.length];
            System.arraycopy(audioConfigByte, 0, keyframe, 0, audioConfigByte.length);
            System.arraycopy(outData, 0, keyframe, audioConfigByte.length, outData.length);
            MainActivity.putAudioData(keyframe, getTimeStamp());
        } else {
            MainActivity.putAudioData(outData, getTimeStamp());
        }
    }
}
